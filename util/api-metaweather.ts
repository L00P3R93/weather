...
export const fetchWeather = async (woeid: number) : Promise<fetchWeatherResponse> => {
    const response = await fetch(`https://www.metaweather.com/api/location/${woeid}/`)
    const { title, consolidated_weather } = (await response.json()) as LocationQueryResponse;
